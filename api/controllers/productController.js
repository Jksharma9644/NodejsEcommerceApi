...
exports.dumpAllProductList = function (req, res) {
  request(
    "https://ecommerce-backend-api.herokuapp.com/productlist/admin",
    function (error, response, body) {
    let res = JSON.parse(body);
    let data = res.data;
    for (var index = 0; index < data.length; index++) {
      var db = new Products(); 
      db.PRODUCT_ID = data[index].PRODUCT_ID;
      db.images = data[index].images;
      db.QTY = data[index].QTY;
      ...
      db.SHIPPINGCHARGES = data[index].SHIPPINGCHARGES;
      db.DISCOUNTRATE = data[index].DISCOUNTRATE;
      db.TAXRATE = data[index].TAXRATE;
      db.NETPRICE = data[index].NETPRICE;
      db.CATEGORY = data[index].CATEGORY;
      db.ISACTIVE = data[index].ISACTIVE;
...
exports.AddProduct = function (req, res) {
  ...
  db.save(function (err, data) {
    if (err) {
      ...
      response = {
        status: true,
        message: "Data added for ID :" + data._id,
...
exports.editProductById = function (req, res) {
  ...
  Products.updateOne(myquery, newvalues, function (err) {
    if (err) {
      ...
      res
        .status(200)
        .send({ status: true, message: "your product has been Edited" });
    }
  });
};
exports.deleteProduct = function (req, res) {
  ...
  Products.findByIdAndRemove({ _id: id }, function (err) {
    if (err) {
      ...
      res
        .status(200)
        .send({ status: true, message: "your product has been DELETED" });
    }
  });
};
